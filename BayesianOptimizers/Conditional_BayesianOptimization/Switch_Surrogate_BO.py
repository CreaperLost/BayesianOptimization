...
from BayesianOptimizers.Conditional_BayesianOptimization.switch_surrogate_per_group import Switch_per_group
...
class Switch_BO:
    ...
    def __init__(
        ...
        for classifier_name in configuration_space:
            ...
            self.object_per_group[classifier_name] = Switch_per_group(f= self.f,lb=None,ub=None,\
    ...
    def run(self): 
        for fold in range(0,self.n_folds):
            ...
            for iter in range(0,self.max_evals_per_fold):
                ...
                #Flip the surrogates only once!
                if self.is_flipped == 0 :
                    flips = np.sum([self.object_per_group[classifier_name].detect_flip() for classifier_name in self.object_per_group])
                    if flips > 0:
                        for classifier_name in self.object_per_group:
                            self.object_per_group[classifier_name].flip_surrogate()
                            self.object_per_group[classifier_name].train_surrogate()
                        changed = 1
                        self.is_flipped =1 
                else:
