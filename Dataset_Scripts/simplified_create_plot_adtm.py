...
for opt in optimizers:
    ...
    for seed in seeds:
        y_per_opt_for_config[opt][seed] = []
        x_per_opt_for_config[opt][seed] = []
        y_per_opt_for_time[opt][seed] = []
        x_per_opt_for_time[opt][seed] = []

dataset_list = []

print(y_per_opt_for_config)
...
for seed in seeds:
    min_per_data[seed] = []
for data_repo in ['AutoML']: #'Jad','OpenML'
    # If the repository doesn't exist then move on.
    path_str = os.path.join(os.pardir,result_space,space_type,'Metric',data_repo)
    if os.path.exists(path_str) == False: continue

    for dataset in os.listdir(path_str):
        id = dataset.split('Dataset')[1] 
        if int(id) not in HALF_CLASSIFICATION_AUTOML_LIST:
            continue
        dataset_name = get_dataset_name_byrepo(dataset,data_repo)
        ...
        for seed in seeds:
            ...
            for opt in optimizers:
                ...
                try:
                    f_path = os.path.join(os.pardir,result_space,space_type,'Metric',data_repo,dataset,'Seed'+str(seed),opt,opt+'.csv')
                    metric=pd.read_csv(f_path,index_col=['Unnamed: 0'])
                    metric.columns = ['Score']
                except:
                    continue
                """time = pd.read_csv(os.path.join(os.pardir,result_space,space_type,'Total_Time',data_repo,dataset,'Seed'+str(seed),opt,opt+'.csv'),index_col=['Unnamed: 0'])  
                ...
                x,y = config_plot_for_opt(metric,opt)
                scores[seed].append(y[-1])
                y_per_opt_for_config[opt][seed].append(y)
                x_per_opt_for_config[opt][seed].append(x)
                """y_per_opt_for_time[opt].append(y_time)
                x_per_opt_for_time[opt].append(x_time)"""
            min_per_data[seed].append( min(scores[seed]) )
...
def compute_row_mean_and_std(dictionary_entry,iter,min_scores,seeds):
    ...
    df_per_seed = {}
    for seed in seeds:
        df_per_seed[seed] = pd.DataFrame()
        for i, arr in enumerate(dictionary_entry[seed]):
            # INVERT
            min_score = - min_scores[seed][i] + 1
            tmp_arr =  (- arr.flatten()) + 1 
            my_array  = (min_score - tmp_arr) *100 / min_score
            print(min_score , tmp_arr , my_array)
            while len(my_array) < iter:
                my_array = np.append(my_array, my_array[-1])
            df_per_seed[seed] [f'Column {i+1}'] = my_array

    f_seed =seeds[0]

    total_df = pd.DataFrame(0,index=np.arange(iter),columns=df_per_seed[f_seed].columns)
    for col in df_per_seed[f_seed].columns:
        for seed in seeds:
            total_df[col] = total_df[col]  + df_per_seed[seed][col]
        
    # Compute the mean of each row
    row_means = total_df.mean(axis=1)
    # Apply the function across each row to calculate the confidence interval
    confidence_intervals = total_df.apply(get_confidence_interval, axis=1, result_type='expand')
    # Rename the columns
...
for opt in optimizers:
    ...
    if opt =='Jad':
        ...
        if opt == 'Greedy_SM':
            ...
            df,result = compute_row_mean_and_std(y_per_opt_for_config[opt],350,min_per_data,seeds)
            print(opt,result['Mean'])
            plt.plot(x,result['Mean'],opt_colors[opt],label=opt)
            #plt.fill_between(x, result['Upper'], result['Low'], color = opt_colors[opt], alpha=.1)
