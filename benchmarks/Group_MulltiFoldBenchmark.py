...
class Group_MultiFold_Space(Group_MultiFold_MLBenchmark):
    def __init__(self,
        ...
        super(Group_MultiFold_Space, self).__init__(task_id, rng, data_path,data_repo,use_holdout,is_multifidelity=is_multifidelity)
    ...
    def optuna_space(self,trial,rng):
        ...
        if classifier_name == 'LinearSVM':
            ...
            model = SVC(C=linear_c,random_state=rng,probability=True)
            ...
            model = SVC(C=rbf_c,gamma=rbf_gamma,random_state=rng,probability=True)
            ...
            model = DecisionTreeClassifier(min_samples_split=2,max_depth=dt_max_depth,min_samples_leaf=dt_min_samples_leaf,random_state=rng)
            ...
            model = RandomForestClassifier(min_samples_split=2,max_depth=rf_max_depth,max_features=rf_max_features,\
            ...
            xgb_n_estimators = trial.suggest_int('xgb_n_estimators', 50, 500, log=False)
            ...
            model = xgb.XGBClassifier(eta=xgb_eta, max_depth=xgb_max_depth, 
    ...
    def init_xgb_multifidelity(self,config : Union[CS.Configuration, Dict],rng : Union[int, np.random.RandomState, None] = None,fidelity=1):
        ...
        new_config['max_depth'] = new_config.pop('XGB_max_depth')
        new_config['n_estimators'] = fidelity #new_config.pop('XGB_n_estimators')
        model = xgb.XGBClassifier(**new_config,**extra_args)
        return model
